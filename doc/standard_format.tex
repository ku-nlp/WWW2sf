...
\subsection{日本語文抽出\label{bunchusyutu}}
...
以下のタグは段落区切りとみなす\\
  hr, p, br, h1, h2, h3, h4, h5, h6, center, div, blockquote, pre,
xmp, listing, plaintext,
    ul, ol, dir, menu, li, dl, dt, dd, table, caption, tr, th, td,
thead, tbody, tfoot, font,
    big, small

\item 2連続(以上)する改行を段落区切りとみなす

\item 段落に含まれる文章中の文字が以下の条件を満たせば文区切りとする

 {\bf 条件:} (括弧内でない \&\& ．である \&\& 両側にアルファベット・数字が現れていな
      い) $||$ 区切り文字である
...
\begin{itemize}
\item 全角文字同士、全角文字と半角文字の間の空白は詰める
\item 連続する空白は詰める
\end{itemize}



\subsubsection{箇条書きの扱い}

\begin{itemize}
\item 文中で箇条書きが列挙されている場合は　を間に挟んで要素を連結 （文
      の外の場合は単独の文として認識される）
\item アルファベットまたは数字の直後に．(ピリオド)がある場合は箇条書きと
      して見なす
\end{itemize}
...
\noindent
S1 次のお店　1. さえずり　2. のら酒房　3. 串カツ屋　は美味しいです。\\
...
\begin{itemize}
\item カタカナに後続するハイフン((ー$|$―$|$−$|$─$|$━$|$‐)を「ー」に正規化
\item ￣を〜へ変換(￣の前後に数字または数字+単位(1から2文字)が現れている
      場合に限り変更)．
\end{itemize}
...
\begin{itemize}
%\itemsep=-1mm
 \item \textbf{「A。(C) B。」のパターンは「A」「(C)」「B」の3つに区切る}
