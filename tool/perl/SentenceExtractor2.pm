...
sub FixParenthesis {
	...
	if ($i > 0) {
	    my $num_of_zenkaku_quote_prev = scalar(split('”', $slist->[$i - 1])) - 1;
	    my $num_of_zenkaku_quote_curr = scalar(split('”', $slist->[$i])) - 1;
	    if ($num_of_zenkaku_quote_prev > 0 && $num_of_zenkaku_quote_curr > 0) {
		if ($num_of_zenkaku_quote_prev % 2 == 1 && $num_of_zenkaku_quote_curr % 2 == 1) {
		    $slist->[$i - 1] .= $slist->[$i];
		    splice(@$slist, $i, $i);
		    redo;
		}
