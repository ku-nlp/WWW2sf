...
GetOptions(\%opt, 'utf8', 'overwrite', 'z', 'force', 'encoding=s');
...
sub main {
    ...
    if ($HtmlGuessEncoding->ProcessEncoding(\$buff, {force_change_to_utf8_with_flag => $opt{force}, change_to_utf8 => !$opt{utf8}, given_encoding => $opt{encoding}})) {
	if ($opt{overwrite}) {
	    if ($opt{z}) {
